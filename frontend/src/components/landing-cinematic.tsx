...
export default function LandingCinematic({ onOpenChamber, announceRef }: Props) {
  ...
  useEffect(() => {
    ...
    import("three").then((T: any) => {
      ...
      const rdr = new T.WebGLRenderer({ canvas, antialias: true, alpha: false });
      ...
      const markers = F.map((f, i) => {
        ...
        ring.style.cssText = `position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:14px;height:14px;border-radius:50%;border:1.5px solid ${f.col};box-shadow:0 0 12px ${f.col}66;transition:width .16s ease,height .16s ease;${isHot ? "animation:kf-ring 2.6s ease-in-out infinite;" : ""}`;
      ...
      const coords = q('[data-readout="coords"]'), veil = q('[data-veil]');
      ...
      if (veil) { veil.style.opacity = "0"; setTimeout(() => { if (veil) veil.style.display = "none"; }, 1200); }
      ...
      const loop = () => {
        ...
        for (let e = 0; e < E; e++) {
          ...
          let cr, cg, cb, ea;
          if (types[e] === 1) { cr = 1.0; cg = 0.27; cb = 0.27; ea = edgeBase * (0.12 + tension * 0.85); }
          else { cr = 0.82; cg = 0.64; cb = 0.36; ea = edgeBase * 0.14 * Math.min(alpha[a], alpha[b]); }
          ecol[e * 6] = cr; ecol[e * 6 + 1] = cg; ecol[e * 6 + 2] = cb; ecol[e * 6 + 3] = cr; ecol[e * 6 + 4] = cg; ecol[e * 6 + 5] = cb;
          ealpha[e * 2] = ea; ealpha[e * 2 + 1] = ea;
        }
        egeo.attributes.position.needsUpdate = true; egeo.attributes.aColor.needsUpdate = true; egeo.attributes.aAlpha.needsUpdate = true;
        ...
        for (let i = 0; i < F.length; i++) {
          ...
          if (hovered === i) { m.ring.style.width = "22px"; m.ring.style.height = "22px"; tip.style.transform = `translate(${Math.min(sx + 18, W - 256)}px,${Math.max(sy - 20, 12)}px)`; tipShown = true; }
          else { m.ring.style.width = "14px"; m.ring.style.height = "14px"; }
        }
        ...
        exploreHint.style.opacity = String(smooth(0.5, 0.58, p) * (1 - smooth(0.7, 0.78, p)));
  ...
  return (
    <div ref={rootRef} aria-hidden className="fixed inset-0 z-10" style={{ fontFamily: SANS }}>
      ...
      <div data-deco="wrap" className="fixed left-1/2 top-1/2 z-[5] pointer-events-none" style={{ transform: "translate(-50%,-50%)", width: 760, height: 480, opacity: 0 }}>
        ...
        <div data-deco="paper" style={{ position: "absolute", left: 380, top: 118, transform: "translate(-50%,-50%)", width: 230, padding: "13px 15px", border: "1px solid rgba(180,196,216,0.4)", borderRadius: 9, background: "linear-gradient(180deg,rgba(16,22,40,0.85),rgba(9,12,24,0.9))", boxShadow: "0 14px 40px rgba(0,0,0,0.5)" }}>
          <div style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: 2, color: "rgba(180,196,216,0.75)" }}>PAPER · arXiv:2403.118</div>
          <div style={{ marginTop: 6, fontSize: 13, fontWeight: 500, lineHeight: 1.3, color: "#eef2fa" }}>Negotiation Outcomes with LLM Coaching</div>
        </div>
        {[
          { l: 150, t: 330, b: "rgba(220,228,245,0.4)", bg: "linear-gradient(180deg,rgba(16,20,30,0.9),rgba(9,12,20,0.92))", tag: "CLAIM · RCT · n=240", tc: "rgba(220,228,245,0.85)", text: "LLM coaching improved negotiation performance.", col: "#eef2fa" },
          { l: 380, t: 392, b: "rgba(200,164,90,0.45)", bg: "linear-gradient(180deg,rgba(28,23,12,0.9),rgba(16,13,8,0.92))", tag: "CLAIM · subgroup · conf 0.79", tc: "rgba(220,180,110,0.95)", text: "Improvement was strongest for novices.", col: "#f4eede" },
          { l: 610, t: 330, b: "rgba(53,208,192,0.45)", bg: "linear-gradient(180deg,rgba(12,26,28,0.9),rgba(8,16,18,0.92))", tag: "CLAIM · longitudinal · 4wk", tc: "rgba(53,208,192,0.95)", text: "The effect persisted after four weeks.", col: "#e6f2f0" },
        ].map((c, i) => (
      ...
      {[
        { s: 0.2, pk: 0.27, e: 0.34, label: "03 / RELATIONSHIPS", line: "Claims connect into evidence.", color: "rgba(200,164,90,0.9)" },
        { s: 0.37, pk: 0.44, e: 0.51, label: "04 / CONTRADICTIONS", line: "ScholarLens finds where evidence disagrees.", color: "rgba(255,120,120,0.92)" },
        { s: 0.55, pk: 0.62, e: 0.69, label: "05 / KNOWLEDGE GRAPH", line: "Claims organize into research fields.", color: "rgba(159,180,216,0.9)" },
      ].map((a, i) => (
        <div key={i} data-phase="anno" data-start={a.s} data-peak={a.pk} data-end={a.e} className="fixed left-1/2 top-1/2 z-[5] pointer-events-none text-center" style={{ transform: "translate(-50%,-50%)", opacity: 0, width: "min(86vw,760px)" }}>
      ...
      {/* stats panel */}
      <div data-stats="panel" className="fixed left-1/2 z-[5] pointer-events-none flex items-end gap-[34px]" style={{ bottom: "8vh", transform: "translateX(-50%)", opacity: 0 }}>
        <div><div style={{ fontFamily: MONO, fontSize: 9, letterSpacing: 3, color: "rgba(150,170,205,0.6)" }}>FIELD</div><div style={{ marginTop: 5, fontSize: 15, fontWeight: 500, color: "#eaf0fb" }}>Negotiation&nbsp;AI</div></div>
        <div style={{ width: 1, height: 36, background: "rgba(140,165,205,0.2)" }} />
      ...
      {/* hypothesis callout */}
      <div data-hyp="wrap" className="fixed left-1/2 top-1/2 z-[5] pointer-events-none" style={{ transform: "translate(-50%,-50%)", opacity: 0 }}>
        <div style={{ position: "relative", width: "min(86vw,640px)", textAlign: "center" }}>
          <div style={{ fontFamily: MONO, fontSize: 11, letterSpacing: 5, color: "rgba(53,208,192,0.95)", marginBottom: 16 }}>06 / HYPOTHESIS</div>
          <div style={{ fontWeight: 300, fontSize: "clamp(28px,4.6vw,58px)", lineHeight: 1.08, letterSpacing: "-0.015em", color: "#eef2fa" }}>A research gap becomes a hypothesis.</div>
          <div style={{ margin: "26px auto 0", width: "min(82vw,460px)", padding: "14px 16px", border: "1px solid rgba(53,208,192,0.5)", borderRadius: 10, background: "linear-gradient(180deg,rgba(10,30,30,0.78),rgba(6,18,18,0.85))", boxShadow: "0 0 40px rgba(53,208,192,0.18), 0 14px 40px rgba(0,0,0,0.5)", textAlign: "left" }}>
            <div style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: 2, color: "rgba(53,208,192,0.95)" }}>GENERATED HYPOTHESIS · H-0427</div>
            <div style={{ marginTop: 6, fontSize: 13.5, lineHeight: 1.4, color: "#def4f0" }}>Coaching gains may reverse under adversarial counterparts — untested across the field.</div>
          </div>
        </div>
      </div>
      ...
      <div data-phase="anno2" className="fixed left-1/2 z-[8] pointer-events-none text-center" style={{ top: "11vh", transform: "translate(-50%,0)", opacity: 0, width: "min(90vw,820px)" }}>
        <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: 5, color: "rgba(159,180,216,0.8)", marginBottom: 12 }}>07 / THE INTERFACE</div>
        <div style={{ fontWeight: 300, fontSize: "clamp(20px,2.8vw,34px)", lineHeight: 1.2, color: "#eef2fa" }}>This universe is built from real literature.<br />ScholarLens is how you navigate it.</div>
      </div>
      ...
      <div data-phase="product" className="fixed inset-0 z-[8] flex items-center justify-center" style={{ opacity: 0, pointerEvents: "none", padding: "4vh 3vw" }}>
        <div style={{ width: "min(96vw,1180px)", height: "min(88vh,720px)", border: "1px solid rgba(159,180,216,0.22)", borderRadius: 14, overflow: "hidden", background: "linear-gradient(180deg,rgba(9,12,22,0.86),rgba(6,8,16,0.92))", backdropFilter: "blur(14px)", boxShadow: "0 40px 120px rgba(0,0,0,0.65)", display: "flex", flexDirection: "column" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "13px 18px", borderBottom: "1px solid rgba(159,180,216,0.16)" }}>
            ...
            <div style={{ marginLeft: "auto", fontFamily: MONO, fontSize: 10, color: "rgba(120,200,140,0.85)" }}>● LIVE · 147 PAPERS MONITORED</div>
          </div>
          <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
            <div style={{ width: 212, borderRight: "1px solid rgba(159,180,216,0.14)", padding: "16px 12px", display: "flex", flexDirection: "column", gap: 2 }}>
              ...
              <div style={{ marginTop: "auto", padding: 11, border: "1px solid rgba(159,180,216,0.25)", borderRadius: 9, background: "rgba(159,180,216,0.06)" }}>
                <div style={{ fontFamily: MONO, fontSize: 8.5, letterSpacing: 2, color: "rgba(170,190,225,0.7)" }}>FIELD</div>
                <div style={{ marginTop: 3, fontSize: 13, color: "#e7eefb" }}>Negotiation&nbsp;AI</div>
              </div>
            </div>
            <div style={{ flex: 1, display: "flex", minWidth: 0 }}>
              ...
              <div style={{ flex: 1, minWidth: 0, padding: 16, display: "flex", flexDirection: "column", gap: 11, overflow: "hidden" }}>
                ...
                <div style={{ padding: "11px 12px", border: "1px solid rgba(255,90,90,0.28)", borderRadius: 9, background: "rgba(255,70,70,0.05)" }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}><span style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 1, color: "#ff9a9a", border: "1px solid rgba(255,90,90,0.4)", borderRadius: 5, padding: "2px 6px" }}>CONTRADICTS</span><span style={{ fontFamily: MONO, fontSize: 9, color: "rgba(170,190,220,0.5)" }}>conf 0.91</span></div>
                  <div style={{ marginTop: 8, fontSize: 12, lineHeight: 1.4, color: "#e9eefa" }}>&ldquo;Coaching gains persist long-term.&rdquo;</div>
                  <div style={{ marginTop: 5, fontSize: 12, lineHeight: 1.4, color: "rgba(220,200,205,0.7)" }}>vs &ldquo;Gains decay within two weeks.&rdquo;</div>
                </div>
                <div style={{ padding: "11px 12px", border: "1px solid rgba(200,164,90,0.3)", borderRadius: 9, background: "rgba(200,164,90,0.04)" }}>
                  <div style={{ display: "flex", gap: 8, alignItems: "center" }}><span style={{ fontFamily: MONO, fontSize: 8, letterSpacing: 1, color: "#e6c690", border: "1px solid rgba(200,164,90,0.4)", borderRadius: 5, padding: "2px 6px" }}>SUPPORTS</span><span style={{ fontFamily: MONO, fontSize: 9, color: "rgba(170,190,220,0.5)" }}>7 papers</span></div>
                  <div style={{ marginTop: 8, fontSize: 12, lineHeight: 1.4, color: "#e9eefa" }}>Novices benefit more than experts.</div>
                </div>
                <div style={{ marginTop: "auto", padding: 12, border: "1px solid rgba(124,111,255,0.4)", borderRadius: 10, background: "rgba(124,111,255,0.08)" }}>
                  ...
                  <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.4, color: "#e4e0ff" }}>Test coaching durability under adversarial counterparts.</div>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* init veil */}
      <div data-veil className="fixed inset-0 z-[12] flex items-center justify-center" style={{ background: "#04050a", transition: "opacity 1.1s ease", pointerEvents: "none" }}>
        <div style={{ fontFamily: MONO, fontSize: 10, letterSpacing: 4, color: "rgba(160,185,225,0.7)" }}>INITIALIZING FIELD…</div>
      </div>
    </div>
  );
}
