...
export default function LibraryPage() {
  ...
  const doSearch = async () => {
    ...
    setSearching(true); setSearchResults([]); setAnswer(""); setSearchError("");
    try { setSearchResults(await api.search(query, 10)); }
  ...
  const doAsk = async () => {
    if (!query.trim()) return;
    setSearching(true); setSearchResults([]); setAnswer(""); setSearchError("");
    try { const r = await api.ask(query); setAnswer(r.answer); }
    catch (e: any) { setSearchError(e.message); }
    setSearching(false);
  };
  ...
  return (
    <div>
      <PageHeader
        title="The corpus"
        subtitle={`${papers.length} paper${papers.length !== 1 ? "s" : ""} · ${totalAnalyses} analyses${totalClaims > 0 ? ` · ${totalClaims} claims extracted` : ""}`}
      />
      ...
      <div className="flex gap-2 mb-3">
        ...
        <button onClick={doSearch}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-[var(--r-md)] bg-[var(--gen)] text-white text-[13px] font-medium t-all hover:opacity-90">
          <Search size={14} /> Search
        </button>
        <button onClick={doAsk}
          className="flex items-center gap-1.5 px-4 py-2.5 rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--surface-2)] text-[13px] text-[var(--text-2)] t-all hover:border-[var(--gen-line)] hover:text-[var(--gen)]">
          <MessageCircleQuestion size={14} /> Ask
        </button>
      </div>

      {searching && <div className="mb-4"><Spinner label="Searching the corpus…" /></div>}

      {searchError && (
        <div className="bg-[var(--contra-dim)] border border-[var(--contra-line)] rounded-[var(--r-md)] px-4 py-3 mb-4 text-[13px] text-[var(--contra)]">
      ...
      {answer && (
        <Card className="mb-6 fade-up">
          <div className="text-[11px] font-medium text-[var(--gen)] uppercase tracking-wider mb-2">Answer</div>
      ...
      {searchResults.length > 0 && (
        <div className="mb-6 space-y-2 fade-up">
          ...
          {searchResults.map((r, i) => {
            const tier = r.relevance_tier ? TIER_LABEL[r.relevance_tier] : null;
            return (
              <div key={i} className="bg-[var(--surface-2)] border border-[var(--line)] rounded-[var(--r-md)] flex overflow-hidden">
                ...
                <div className="p-3.5 pl-4 flex-1">
                  <div className="flex justify-between items-center mb-1.5">
                    <span className="text-[10px] text-[var(--text-4)] uppercase tracking-wider">{r.section || "general"}</span>
                    {tier ? (
                      <span className="text-[11px] font-medium" style={{ color: tier.color }}>{tier.label}</span>
                    ) : (
                      <span className="mono text-[11px] text-[var(--gen)]">{r.relevance}%</span>
                    )}
                  </div>
      ...
      <div className="flex items-center gap-2 mb-4">
        ...
        <button onClick={() => setSortBy((s) => s === "recent" ? "coverage" : "recent")}
        ...
        <div className="grid grid-cols-[1fr_320px] gap-4">
          ...
          <div className="bg-[var(--surface-2)] border border-[var(--line)] rounded-[var(--r-lg)] overflow-hidden">
            <div className="grid grid-cols-[1fr_auto_auto] gap-4 px-4 py-2.5 border-b border-[var(--line)] text-[10px] uppercase tracking-wider text-[var(--text-4)] font-medium">
              <span>Paper</span><span>Analyses</span><span>Coverage</span>
            </div>
            {filtered.map((p) => {
                ...
                <button key={p.id} onClick={() => setSelected(p)}
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[11.5px] text-[var(--text-3)] mt-0.5">
                      {(p.authors || []).slice(0, 2).join(", ")} · {p.year || "?"}
                    </div>
                  </div>
                  <span className="mono text-[12px] text-[var(--text-2)] tabular-nums">{p.analysis_types?.length || 0}/6</span>
            ...
            {selected ? (
              <Card>
                ...
                <div className="text-[12px] text-[var(--text-3)] mb-4">
                  {(selected.authors || []).slice(0, 3).join(", ")} · {selected.year || "?"}
                </div>
                {selected.abstract && (
                  <div className="mb-4">
                    <div className="text-[11px] font-medium text-[var(--text-3)] uppercase tracking-wider mb-1.5">Abstract</div>
                    <p className="text-[12px] text-[var(--text-2)] leading-[1.6]">
                      {selected.abstract.slice(0, 280)}{selected.abstract.length > 280 ? "…" : ""}
                    </p>
                  </div>
                )}
                <div className="text-[11px] font-medium text-[var(--text-3)] uppercase tracking-wider mb-2">Analyses</div>
                <div className="flex flex-wrap gap-1 mb-4">
                  {(selected.analysis_types || []).map((t) => <AnalysisTag key={t} type={t} />)}
                  {(selected.analysis_types?.length || 0) === 0 && (
                    <span className="text-[12px] text-[var(--text-3)]">None yet</span>
                  )}
                </div>
                <a href={`/paper/${selected.id}`}
                  className="block text-center py-2 rounded-[var(--r-md)] border border-[var(--line)] text-[12.5px] text-[var(--gen)] font-medium t-all hover:border-[var(--gen-line)] hover:bg-[var(--gen-dim)]">
                  Open paper →
                </a>
              </Card>
            ) : (
              <Card>
                <div className="text-[13px] text-[var(--text-3)] text-center py-8">Select a paper to inspect it.</div>
              </Card>
            )}
          </div>
        </div>
