  ...
  const handleExport = () => {
    ...
    const rows = hypotheses.map((h, i) => `
      <div class="hypothesis">
        <div class="h-num">H${i + 1}</div>
        <h2>${h.statement}</h2>
        <p class="rationale">${h.rationale}</p>
        ${h.methodology ? `<div class="section-label">Methodology</div><p>${h.methodology}</p>` : ""}
        ${h.challenges?.length ? `<div class="section-label">Anticipated Challenges</div><ul>${h.challenges.map((c: string) => `<li>${c}</li>`).join("")}</ul>` : ""}
        ${h.supporting_papers?.length ? `<div class="section-label">Source Papers</div><ul>${h.supporting_papers.map((sp: any) => `<li><strong>${sp.title}</strong>${sp.relevant_finding ? ` — ${sp.relevant_finding}` : ""}</li>`).join("")}</ul>` : ""}
      </div>`).join("");
    const html = `<!DOCTYPE html><html><head><meta charset="utf-8"><title>Research Hypotheses — ScholarLens</title>
<style>
  body { font-family: Georgia, serif; font-size: 13pt; line-height: 1.7; color: #1a1a1a; max-width: 720px; margin: 0 auto; padding: 48px 40px; }
  h1 { font-size: 22pt; font-weight: 700; margin: 0 0 4px 0; letter-spacing: -0.02em; }
  .meta { color: #666; font-size: 10pt; margin-bottom: 40px; padding-bottom: 16px; border-bottom: 1px solid #ddd; }
  .hypothesis { margin-bottom: 40px; padding-bottom: 40px; border-bottom: 1px solid #e8e8e8; page-break-inside: avoid; }
  .hypothesis:last-child { border-bottom: none; }
  .h-num { font-size: 9pt; font-weight: 700; letter-spacing: 0.08em; color: #7c5cbf; text-transform: uppercase; margin-bottom: 6px; }
  h2 { font-size: 15pt; font-weight: 700; margin: 0 0 12px 0; line-height: 1.35; letter-spacing: -0.01em; }
  .rationale { color: #444; margin: 0 0 16px 0; }
  .section-label { font-size: 9pt; font-weight: 700; letter-spacing: 0.08em; text-transform: uppercase; color: #999; margin: 16px 0 6px 0; }
  ul { margin: 0 0 12px 0; padding-left: 20px; }
  li { margin-bottom: 6px; color: #333; }
  @media print { body { padding: 24px; } }
</style></head><body>
<h1>Research Hypotheses</h1>
<div class="meta">Generated by ScholarLens · ${date}</div>
${rows}
</body></html>`;
