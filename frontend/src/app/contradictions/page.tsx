...
import { PageHeader, Card, EmptyState, Spinner, Slider, SelectChip, PrimaryButton, Claim, REL } from "@/components/ui";
import { Zap, Settings2, X } from "lucide-react";
...
export default function ContradictionsPage() {
  ...
  const runScan = async () => {
    setLoading(true); setResults([]); setFilter(null); setSelected(null);
    const stages = ["Extracting claims from papers", "Embedding & comparing claims", "Judging relationships"];
    let si = 0; setStage(stages[0]);
    const ticker = setInterval(() => { si = Math.min(si + 1, stages.length - 1); setStage(stages[si]); }, 2500);
    try {
      const res = await api.runContradictions({ paperIds: selectedIds.length ? selectedIds : undefined, similarityThreshold: threshold, maxPairs });
      setResults(res); setShowConfig(false);
  ...
  return (
    <div>
      <PageHeader title="Conflict map" subtitle="Where your papers disagree — claim against claim."
        action={results.length > 0 && (
          <button onClick={() => setShowConfig((s) => !s)} className="flex items-center gap-1.5 px-3 py-2 rounded-[var(--r-md)] border border-[var(--line)] bg-[var(--surface-2)] text-[12.5px] text-[var(--text-2)] t-all hover:border-[var(--line-2)] hover:text-[var(--text-1)]">
            <Settings2 size={14} /> Scan settings
          </button>
        )} />
        ...
        <>
          {showConfig && (
            <Card className="mb-6 fade-up">
              <div className="text-[11px] font-medium text-[var(--text-3)] uppercase tracking-wider mb-2.5">Select papers · empty = all</div>
              <div className="flex flex-wrap gap-1.5 mb-5">
                {papers.map((p) => (
                  <SelectChip key={p.id} label={p.title.length > 44 ? p.title.slice(0, 44) + "…" : p.title}
                    active={selectedIds.includes(p.id)}
                    onClick={() => setSelectedIds((ids) => ids.includes(p.id) ? ids.filter((x) => x !== p.id) : [...ids, p.id])} />
                ))}
              </div>
              <div className="grid grid-cols-2 gap-6 mb-5">
                <Slider label="Similarity threshold" value={threshold} min={0.3} max={0.9} step={0.05} onChange={setThreshold} format={(v) => v.toFixed(2)} />
                <Slider label="Max comparisons" value={maxPairs} min={5} max={30} step={1} onChange={(v) => setMaxPairs(Math.round(v))} />
              </div>
              <PrimaryButton onClick={runScan} disabled={loading}><Zap size={15} /> {loading ? "Scanning…" : "Run analysis"}</PrimaryButton>
            </Card>
          )}

          {loading && <Card className="mb-6"><Spinner label={stage} /></Card>}

          {results.length > 0 && (
            <div className="fade-up">
              {/* Summary counts as filters */}
              ...
              {/* Split: tension pairs + adjudication rail */}
...
/* Two claims facing each other, joined by a charged line */
function TensionPair({ r }: { r: ContradictionResult }) {
  const s = REL[r.relationship];
  const charged = r.relationship === "contradiction";
  return (
    <div>
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="ml-auto mono text-[10px] text-[var(--text-4)]">sim {(r as any).similarity?.toFixed?.(2) ?? "—"}</span>
      </div>
      <div className="grid grid-cols-[1fr_auto_1fr] gap-3 items-center">
        ...
        <div className="flex flex-col items-center w-12">
          <div className={`h-[2px] w-full ${charged ? "charged" : ""}`} style={{ background: s.line }} />
        </div>
...
/* Right rail: streamed adjudication */
function Adjudication({ r }: { r: ContradictionResult }) {
  ...
  const stronger = r.stronger_evidence === "paper_a" ? r.claim_a.paper_title : r.stronger_evidence === "paper_b" ? r.claim_b.paper_title : null;
